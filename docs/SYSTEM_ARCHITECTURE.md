# 🛠 SYSTEM ARCHITECTURE: Muon Blueprint

Muon is built on a 3-layer architecture that separates physical hardware control, the API gateway logic, and the user-facing interface.
...
- **Hardware Peripherals:**
    ...
    - **Touch Sensor:** Trigger for `TOUCH_EVENT` (Muon reacts to pets).

## 2. Gateway Layer (Node.js)
- **Role:** The central nervous system. Manages all states and routes messages.
- **Key Logic:**
    - **WebSocket Server:** Bridges the ESP32 and Muon Hub.
    - **AI Context Builder:** Injects real-time Muon Hub data (Mood, Track, Weather) into Ollama's system prompt.
...
| :--- | :--- | :--- |
| `CHAT_REQUEST` | Muon Hub | User sends a text message for Muon's brain. |
| `CHAT_RESPONSE` | Gateway | Muon's AI response to be displayed in chat. |
| `MOOD_CHANGE` | Gateway | Tells ESP32 which hardware animation to play. |
| `MOOD_UPDATE` | Gateway | Synchronizes the Muon Hub UI face with the current mood. |
| `TOUCH_EVENT` | ESP32 | Triggered when Abrar "pets" the physical bot. |
| `SPOTIFY_CONTROL` | Muon Hub | `play`, `pause`, `next`, `previous` actions. |
| `RAIN_WARNING` | Gateway | Sent when OpenWeather predicts rain soon. |

---
> [!IMPORTANT]
> All communication is real-time. If the ESP32 disconnects, the Muon Hub sidebar will show a **SYSTEM_OFFLINE** status.
